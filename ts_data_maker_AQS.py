...
aqs_data = np.load(path+'aqs_o3_2016.npy')
            ...
            for k in range(4):
                for l in range(5):
                    if 4*i+k < lat_orb and 5*j+l < lon_orb:
                        if aqs_data[n,4*i+k,5*j+l] > 10 and aqs_data[n,4*i+k,5*j+l] < 100:
...
dataset = Dataset(path+'aqs_o3_gc_2016.nc','w',format='NETCDF4_CLASSIC')
