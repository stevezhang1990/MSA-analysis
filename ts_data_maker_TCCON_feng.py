...
from scipy import *
from pylab import *
# read in ND49 for different experiments
# read in sample files for lat,lon,dim and pressure information
infile00 = Dataset('/users/jk/16/xzhang/gcadj_std_M_V35/runs/v8-02-01/geos5_mop_1601/ctm.01.20160101.nc')
ts_ap = Dataset('/users/jk/16/xzhang/TS_annual/geos5_omino2_2016/ts_omino2_2016.nc')
ts_mop = Dataset('/users/jk/16/xzhang/TS_annual/geos5_mop_2016/ts_mop_2016.nc')
ts_msa05 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_07_2016/ts_all_07_2016.nc')
ts_msa06 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_08_2016/ts_all_08_2016.nc')
strat_ap = Dataset('/users/jk/16/xzhang/TS_annual/geos5_ap_2016/co_strat_ap_2016.nc')
strat_mop = Dataset('/users/jk/16/xzhang/TS_annual/geos5_mop_2016/co_strat_mop_2016.nc')
strat_msa05 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_07_2016/co_strat_msa07_2016.nc')
strat_msa06 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_08_2016/co_strat_msa08_2016.nc')
ts_pres = Dataset('/users/jk/16/xzhang/TS_annual/geos5_ap_2016/ts_pres_2016.nc')
print "check"
# combine tropospheric CO and stratospheric CO profiles
len_time,len_trop,len_lat,len_lon = shape(ts_ap.variables["ts_co"][:,:,:,:])
len_lvl = 47
ts_co = np.zeros((4,len_time,len_lvl,len_lat,len_lon))
ts_co[0,:,0:len_trop,:,:] = ts_ap.variables["ts_co"][:,:,:,:]
ts_co[1,:,0:len_trop,:,:] = ts_mop.variables["ts_co"][:,:,:,:]
ts_co[2,:,0:len_trop,:,:] = ts_msa05.variables["ts_co"][:,:,:,:]
...
# lat, lon, pressure edge and center info
...
for i in range(12):
   ...
   # READ TCCON FILE
      FILE_NAME='/users/jk/16/dengf/obs/TCCON_2016_CO/tcco_xco_'+year+month+day+'.dat'
      print FILE_NAME
      file_counter=0
      if os.path.isfile(FILE_NAME):
         ...
         # LOCATE TCCON OBS: CO MIXING RATIO AND PRESSURE IN GC MODEL
            ts_co_col[4,ts_i,lat_i,lon_j] = XCO[file_i]
            for exp_i in range(4):
               ...
               GC_CO_NATIVE[:len_trop]=ts_co[exp_i,ts_i,:len_trop,lat_i,lon_j]
            ...
            # MAPPING FROM GC LVLS TO TCCON LVLS, NEED #GC_LVLS,PCENTER ON TCCON GRID, GC_PSURF, #TCCON LVLS, TCCON PRESSURE, TCCON PSURF
               MAP[:,:]=get_intmap(len_lvl,ts_p_center[ts_i,:,lat_i,lon_j], \
                                   gc_psurf, obs_lvl,tccon_pressure[file_i,:], tccon_psurf[file_i],mode='MR')
...
# save tccon data and experiment values overlapping with TCCON sites in NetCDF files
dataset = Dataset('/users/jk/16/xzhang/TCCON/tccon_gc_2016_feng.nc','w',format='NETCDF4_CLASSIC')
