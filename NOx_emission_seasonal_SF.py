...
NOX_an = np.zeros((8,len1,len2))
NOX_bb = np.zeros((8,len1,len2))

SF_omi_an = np.zeros((8,len1,len2))
SF_omi_bb = np.zeros((8,len1,len2))
...
title('NOx emissions: Jan-April [1e9 molec/cm3/s]')
...
#For NOx 
anth_plot = m1.pcolormesh(x,y,plot_data/1e9,vmin=1,vmax=120,cmap=get_cmap("hot_r"))
...
ax = subplot(232)
title('NOx emissions SF: OMI only')
plot_data = np.mean(NOx_omi[:,:,:],axis=0)
m1=Basemap(lon_0=0)
m1.drawcoastlines()
m1.drawparallels(arange(-90,120,30),labels=[1,0,0,1],labelstyle="+/-")
m1.drawmeridians(arange(-180,210,60),labels=[1,0,0,1],labelstyle="+/-")
...
ax = subplot(233)
title('NOx emissions SF: All instrument')
plot_data = np.mean(NOx_all[:,:,:],axis=0)
m1=Basemap(lon_0=0)
m1.drawcoastlines()
m1.drawparallels(arange(-90,120,30),labels=[1,0,0,1],labelstyle="+/-")
m1.drawmeridians(arange(-180,210,60),labels=[1,0,0,1],labelstyle="+/-")
