...
ts_omi = Dataset('/users/jk/16/xzhang/TS_annual/geos5_omino2_msa1/ts_omino2_msa1.nc')
ts_msa05 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_05_msa1/ts_all_05_msa1.nc')
gome2_data = Dataset('/users/jk/16/xzhang/GOME2_NO2/gome2_msa1.nc')
#ts_msa06 = Dataset('/users/jk/16/xzhang/TS_annual/geos5_all_06_2016/ts_all_06_2016.nc')
...
ts_no2_col = np.zeros((4,len_time,len_lat,len_lon))
vcdt_gc = gome2_data.variables["vcdt_gc"][:,:,:]
vcd_gc = gome2_data.variables["vcd_gc"][:,:,:]
pcenter_gc = ts_pres.variables["ts_p_center"][:,:,:,:]
pedge_gc = ts_pres.variables["ts_p_edge"][:,:,:,:]
...
NO2_gc_annual = np.zeros((3,len_time,len_lvl,len_lat,len_lon))
NO2_gc_annual[0,:,:,:,:] = ts_ap.variables["ts_no2"][:,:,:,:]
NO2_gc_annual[1,:,:,:,:] = ts_omi.variables["ts_no2"][:,:,:,:]
NO2_gc_annual[2,:,:,:,:] = ts_msa05.variables["ts_no2"][:,:,:,:]

gc_p_edge = ts_pres.variables["ts_p_edge"][:,0:len_lvl+1,:,:]
...
for i in range(len_time):
    for j in range(len_lat):
        for k in range(len_lon):
            if vcdt_gc[i,j,k] > 0:
                ts_no2_col[3,i,j,k] = vcdt_gc[i,j,k]*1e15
                ...
                for exp_i in range(3):
                    ...
                    if i < 24*13:
                        gc_no2_native = NO2_gc_annual[exp_i,i,:,j,k]*1e-9*airden_gc[i%24,:,j,k]*bxh_gc[i%24,:,j,k]*100
                    else:
                        gc_no2_native = NO2_gc_annual[exp_i,i,:,j,k]*1e-9*airden_gc[24+i%24,:,j,k]*bxh_gc[24+i%24,:,j,k]*100
...
dataset = Dataset('/users/jk/16/xzhang/GOME2_NO2/gome2_gc_msa1.nc','w',format='NETCDF4_CLASSIC')
...
ts_no2_msa05_tccon = dataset.createVariable('ts_no2_msa05_gome2',np.float32,('time','lat','lon'))
ts_no2_msa05_tccon[:,:,:] = ts_no2_col[2,:,:,:]
ts_no2_obs_tccon = dataset.createVariable('ts_no2_obs_gome2',np.float32,('time','lat','lon'))
ts_no2_obs_tccon[:,:,:] = ts_no2_col[3,:,:,:]
dataset.close()
print "done"
show()
